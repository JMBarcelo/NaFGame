...
$(document).ready(function() {
  ...
  for (i = 2; i < rows-2; i++) {
    for (j = 2; j < cols-2; j++) {
      if (document.getElementById(pad(j+1) + pad(i)).classList.contains('empty_tile') && document.getElementById(pad(j) + pad(i+1)).classList.contains('empty_tile') && document.getElementById(pad(j-1) + pad(i)).classList.contains('wall_tile') && document.getElementById(pad(j) + pad(i-1)).classList.contains('wall_tile')) {
        document.getElementById(pad(j) + pad(i)).classList.add('tt_in_tile');
        document.getElementById(pad(j) + pad(i)).classList.add(pad('tt_in_' + pad(tt_in)));        
        document.getElementById(pad(j) + pad(i)).classList.remove('empty_tile');
        tt_in++;
      }
    }
  ...
  for (i = 2; i < rows-2; i++) {
    for (j = 2; j < cols-2; j++) {
      if (document.getElementById(pad(j-1) + pad(i)).classList.contains('empty_tile') && document.getElementById(pad(j) + pad(i-1)).classList.contains('empty_tile') && document.getElementById(pad(j+1) + pad(i)).classList.contains('wall_tile') && document.getElementById(pad(j) + pad(i+1)).classList.contains('wall_tile')) {
        ...
        document.getElementById(pad(j) + pad(i)).classList.add(pad('tt_out_' + pad(tt_out)));        
        document.getElementById(pad(j) + pad(i)).classList.remove('empty_tile');
        tt_out++;
      }
    }
  ...
  for (i = 0; i < tt_in; i++) {
    ...
    roomArray = [];
    for(j = parseInt(posY); document.getElementById(posX + pad(j)).className != 'wall_tile';) {
      rowArray = [];
      for(var k = parseInt(posX); document.getElementById(pad(k) + pad(j)).className != 'wall_tile'; k++) {   
  ...
  function revealRoom(moor) {
    for (i = 0; i < allRoomsArray[moor].length; i++) {
      for (j = 0; j < allRoomsArray[moor][i].length; j++) {
        document.getElementById(allRoomsArray[moor][i][j]).classList.remove('invis')
      }
    }
  }
  ...
  function scroll() {
    window.scrollTo(warrior.getBoundingClientRect().right - window.innerWidth / 2, warrior.getBoundingClientRect().bottom - window.innerHeight / 2);
  }
  ...
  var coords = parseInt(document.getElementsByClassName('tt_in_00')[0].id) + 100;
  if (coords < 1000) {
    coords = '0' + coords
  } else {
    coords = '' + coords
  ...
  $(document).on('keydown', function(event) {
    if ( event.which == 65 && parseInt(posX) - 1 >= 0) {
      ...
      if (document.getElementById(posX + posY).classList.contains("empty_tile")){
        ...
        warrior = document.getElementById(posX + posY)
        setWarrior(warrior);
      } else if (document.getElementById(posX + posY).classList.contains("tt_out_tile")) {
        cleanWarrior(warrior);
